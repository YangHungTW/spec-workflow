...
WORKTREE="/Users/yanghungtw/Tools/spec-workflow/.worktrees/symlink-operation-T3"
...
actual_output=$(cd /tmp && "$SCRIPT" __probe 2>/dev/null)
...
actual_output=$(/tmp/cs-link __probe 2>/dev/null)
if [ "$actual_output" = "$expected_output" ]; then
...
assert_exit_nonzero \
  ...
  "$SCRIPT" __probe resolve /tmp/cycle_A
