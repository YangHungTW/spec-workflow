...
- 2026-04-21 implement — orchestrator cwd-drift incident: 10 W3 batch-1 merges inadvertently landed on branch 20260421-rename-to-specaffold-T20 instead of main feature branch (prior `cd .worktrees/T20` for T20 typo fix persisted across bash calls); recovered via `git merge --ff-only 20260421-rename-to-specaffold-T20` from main worktree; all 10 merge commits (7dd4a88..a7466aa) intact
- 2026-04-21 implement — T19 typo followup commit 80df49b: 3 rule files had `specflow → specaffold` for path/binary refs where it should have been `→ scaff`; 7 instances corrected (bin/scaff-install-hook, bin/scaff-*, .claude/agents/scaff) in language-preferences.md / no-force-on-user-paths.md / absolute-symlink-targets.md
- 2026-04-21 implement — T20 typo orchestrator fix (amended in T20 worktree before merge): `bin/specaffold-run → bin/scaff-run` on reviewer/security.md:56
- 2026-04-21 review result — wave 3 batch 1 verdict=NITS (T18 style: line 95 spacing; T21c security advisory: pre-existing hardcoded dev path in t10_summary.sh); batch 2 verdict=PASS; batch 3 verdict=PASS
- 2026-04-21 implement wave 3 done — T17, T18, T19, T20, T21a, T21b, T21c, T22, T23, T24, T25, T26, T27 (W3 merged; 13 tasks done; 4/33 tasks remain for W4)
- 2026-04-21 implement — PLAN GAP surfaced by t_grep_allowlist.sh dry-run: T28 will fail hard — 3604 violation lines across 8 non-flow-monitor files (test/t_T25_ensure_compat_symlink.sh, test/t_grep_allowlist.sh, bin/scaff-seed, .claude/team-memory/README.md, .gitignore, scripts/tier-rollout-migrate.sh, .specaffold/features/20260421-rename-to-specaffold/*.md, .specaffold/features/_template/STATUS.md) PLUS the entire flow-monitor/ subtree (source + build artefacts); user decision required before W4
- 2026-04-21 update-plan — TPM added T21d: T28 gap closeout combining rewrite of 6 missed files (.gitignore, .claude/team-memory/README.md, scripts/tier-rollout-migrate.sh, bin/scaff-seed residual, test/t_T25_ensure_compat_symlink.sh, .specaffold/features/_template/STATUS.md) + allow-list extension for self-reference (test/t_grep_allowlist.sh) + feature docs (.specaffold/features/20260421-rename-to-specaffold/**) + flow-monitor subtree (.gitignore build artefacts flow-monitor/src-tauri/target + flow-monitor/dist AND allow-list source flow-monitor/**). W3 task count 13→14, total 33→34. Dep: all T17–T27 merged (confirmed in e3a8a0b).
- 2026-04-21 T21d retry scope — dev correctly preserved semantic `.spec-workflow` references in bin/scaff-seed and test/t_T25_ensure_compat_symlink.sh (the legacy compat-link name is the subject of those files, not an obsolete name); added both files to allow-list as additional entries. Commit 2856f8c then amended to 9b68cc2 for reviewer nits (two comment typos `.specaffold → .specaffold` corrected to `.spec-workflow → .specaffold` in bin/scaff-seed)
