...
- 2026-04-26 archive — branch merged to main (ed8d05d) and pushed to origin/main; merge-check passes. Retrospective approved 4 of 5 TPM proposals: qa-tester/subprocess-run-required-for-bash-portability-claim (local), tpm/skip-inline-review-must-be-pre-authorized-in-plan (local), developer/architectural-shortcut-must-mirror-to-plan-verify (local), qa-tester/sandbox-home-needs-path-pin-when-tooling-uses-asdf (global, ~/.claude/team-memory/qa-tester/). Dropped: pm/prd-ac-grep-count-must-match-plan-section (covered by existing memory entries). Out-of-band fix: added .claude/settings.json (project-level SessionStart hook registration) so .claude/hooks/session-start.sh actually fires; user-level had no hooks block, hence LANG_CHAT=zh-TW and rule banner injection never reached this session.
